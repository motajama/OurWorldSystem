...
The productive capability proxy writes `static/data/indicators/productive-capability.world-bank.latest.json` from the same WDI export-structure values. It uses manufactures exports, high-technology exports, and medium/high-technology exports as a limited positive structural support signal. It is not final productive complexity and does not measure value-chain control, value capture, ownership, or domestic value added. Manufacturing and high-tech export shares are useful because they identify industrial capability, but they can also reflect dependent assembly, FDI-led production, enclave exports, or re-export/platform effects.
...
Derived records can be provisional `core` only when `quality_of_life_score >= 0.90`, `productive_capability_score >= 85`, productive capability data quality is `good`, `extraction_dependency_score <= 20`, `extraction_autonomy_score >= 70`, high-tech export depth is present (`high_tech_exports_manufactured_pct >= 20` or `medium_high_tech_exports_manufactured_pct >= 65`), direct evidence fields are present, the industrial-semiperiphery guard is false, and the record is not disputed/special/territory. Demo seed records cannot become core. If a legacy demo seed says `core` without a curated override, the provisional output is low-confidence `semi-periphery` with `source: "legacy_demo_seed_reinterpreted"`. If quality is very high but productive capability evidence is missing or insufficient, the record is normally low-confidence `semi-periphery` with the explanation that positive productive capability evidence is missing or insufficient. A continuous proxy score may be high while the class remains `semi-periphery`.

The provisional `industrial_semiperiphery_guard` downgrades likely dependent manufacturing/export-platform cases to `semi-periphery` with `profile: "industrial_semiperiphery"` until TiVA/GVC/value-capture evidence is added. The transparent provisional watchlist is `CRI`, `HUN`, `LTU`, `POL`, `SVN`, and `EST`; it is a methodological safeguard, not a final classification. Costa Rica-, Hungary-, Poland-, Lithuania-, Slovenia-, and Estonia-type cases can show strong manufacturing or medium/high-tech export shares while still lacking evidence of domestic value capture or lead-firm control. The final structural model needs OECD TiVA, Atlas/BACI/Comtrade, domestic value added, ownership, and GVC-position indicators to adjudicate these cases.
