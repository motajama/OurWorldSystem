...
- `core`: for derived records, requires `quality_of_life_score >= 0.90`, `productive_capability_score >= 85`, productive capability data quality `good`, `extraction_dependency_score <= 20`, `extraction_autonomy_score >= 70`, high-tech export depth (`high_tech_exports_manufactured_pct >= 20` or `medium_high_tech_exports_manufactured_pct >= 65`), all direct evidence fields present, no `industrial_semiperiphery_guard`, and no disputed/special/territory status unless explicitly reviewed in a curated override. Demo seeds cannot create core.
...
The `industrial_semiperiphery_guard` exists because WDI manufacturing and high-tech export shares can show industrial capability without showing domestic value capture, ownership, lead-firm control, or GVC position. It can identify dependent manufacturing, assembly, enclave exports, FDI-led production, or re-export/platform effects. Guarded productive-capability candidates are downgraded to `semi-periphery` with `profile: "industrial_semiperiphery"` and an explanation that core status requires value-capture/GVC evidence.

The guard currently triggers, when value-capture/GVC evidence is absent, for high manufacturing share with modest high-tech depth, high medium/high-tech share with high-tech below 20 percent, structurally significant food exports, or membership in the transparent provisional watchlist `CRI`, `HUN`, `LTU`, `POL`, `SVN`, `EST`. The watchlist is not final truth; it is a removable review safeguard for Costa Rica-, Hungary-, Poland-, Lithuania-, Slovenia-, and Estonia-type cases pending OECD TiVA, Atlas/BACI/Comtrade, domestic value-added, ownership, and GVC-position evidence.
...
The builder also prints diagnostics: total records, class distribution, core records with evidence, derived core count, derived productive-capability core count, curated reviewed core count, demo seed reinterpretation count, top productive capability scores, high-score countries kept semi-periphery because productive capability is missing/low, countries moved into provisional core by productive capability support, records downgraded by `industrial_semiperiphery_guard`, records kept as core despite warnings, all `industrial_semiperiphery` records, all `core_like_semiperiphery` records, and top downgraded high-quality records with reasons.
...
This is provisional positive structural support only. It measures export structure, not domestic value capture, ownership, GVC control, or final productive complexity. It should later be replaced or complemented by Atlas, BACI, Comtrade, and OECD TiVA evidence. Manufacturing/high-tech export shares are informative but insufficient: strong scores can be produced by dependent industrial integration rather than core value-chain control.
