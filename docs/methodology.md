...
## Provisional World-System Proxy

The current default world-system layer is provisional, derived, experimental, and marked as needing review. It is generated by `scripts/data/build-provisional-world-system.mjs` into `static/data/indicators/world-system.provisional.latest.json`.

The builder preserves existing demo records from `static/data/world-system.latest.json` as `source: "demo_curated"` so curated/demo classes are not silently overridden by the proxy. All other registry records are derived from currently available World Bank WDI quality-of-life data as `source: "derived_world_bank_quality_proxy"`.

The provisional score is on a 0-100 scale. It starts from the World Bank-derived `quality_of_life_score` where available. When GNI per capita PPP is present, the model applies a small normalized log-income adjustment. Current provisional bins are:

- `core`: score >= 78
- `semi-periphery`: score >= 55 and < 78
- `periphery`: score < 55
- `uncertain`: insufficient or contradictory quality/income signals
- `disputed`: disputed map units without stable comparable data
- `no_data`: missing values

These thresholds are temporary visualization thresholds. They are not a final Wallersteinian classification and should not be treated as a claim about a map unit's complete structural position in the capitalist world-system. The model currently overweights quality-of-life and income proxies because those are the public, reproducible data available in the initial static pipeline.

A defensible future model should include evidence families that this proxy does not yet include: OECD TiVA, trade and value-chain data, material footprint, e-waste, ecological externalization, military/geopolitical position, financial centrality, conflict exposure, and political-freedom indicators. Those additions should preserve source years, transformation notes, uncertainty, and neutral handling of disputed map units.
